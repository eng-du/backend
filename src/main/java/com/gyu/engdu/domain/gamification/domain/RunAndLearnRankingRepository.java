...
public interface RunAndLearnRankingRepository extends JpaRepository<RunAndLearnRanking, Long> {
        ...
        Optional<RunAndLearnRanking> findByUserIdAndRankingTypeAndSeason(Long userId, RankingType rankingType,
                        int season);

        /**
         * 특정 랭킹 타입과 시즌에서, 입력받은 점수보다 높은 점수를 기록한 유저의 수를 반환합니다.
         * SELECT count(*)
        ...
        /**
         * 특정 랭킹 타입과 시즌에서 최고 점수 순, 달성 시간 순으로 랭킹을 조회합니다. (Pageable을 통해 갯수 조절)
         * SELECT *
         * FROM run_and_learn_ranking
         * WHERE ranking_type = ? AND season = ?
         * ORDER BY best_score DESC, achieved_at ASC
         * LIMIT ?
         */
        List<RunAndLearnRanking> findByRankingTypeAndSeasonOrderByBestScoreDescAchievedAtAsc(RankingType rankingType,
